<x-app-layout>
    ...
    <x-panel-page title="متابعة الأبناء" subtitle="الدرجات، المصاريف، والرسائل — لكل ابن من بطاقة واحدة.">
        <x-slot:actions>
        ...
        <div class="dashboard-hero">
            <div class="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                ...
                <a href="{{ route('parent.exams') }}" class="hero-btn-ghost" wire:navigate>كل النتائج</a>
            </div>
        </div>
        ...
        @if ($stats['attention_count'] > 0)
            <div class="attention-strip">
                ...
                @if ($stats['unread_messages'] > 0)
                    <a href="{{ route('parent.messages') }}" class="rounded-lg bg-white px-2.5 py-1 text-xs font-semibold ring-1 ring-amber-200" wire:navigate>
                ...
                @foreach ($stats['children'] as $child)
                    @if ($child['pending_subscriptions'] > 0 || $child['pending_payments'] > 0)
                        <a href="{{ route('parent.children.payments', $child['id']) }}" class="rounded-lg bg-white px-2.5 py-1 text-xs font-semibold ring-1 ring-amber-200" wire:navigate>
                            دفع — {{ $child['name'] }}
                        </a>
                    @endif
                @endforeach
        ...
        <section>
            <div class="section-head">
                <div>
                    <h2 class="section-title">ماذا تفعل؟</h2>
                    <p class="section-subtitle">ثلاث خطوات واضحة لمتابعة ابنك في السنتر.</p>
                </div>
            </div>
        ...
        <x-panel-card title="الأبناء" subtitle="المجموعة، المصاريف، الحضور، والنتائج.">
            <div class="space-y-3">
